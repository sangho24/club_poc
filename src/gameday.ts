 ...
 * 구단 공식 예매 안내 페이지.
 *
 * 홈의 '예매' 지름길이 여기로 간다. 직관 탭의 예매 버튼(`TICKET_CHANNEL.url`)은 **고른
 * 경기 하나**를 티켓링크 판매 페이지로 넘기는 것이고, 홈의 지름길은 아직 경기를 고르기
 * 전이다. 고르지도 않은 채로 판매 페이지에 떨어뜨리면 팬이 거기서 다시 경기를 찾아야
 * 한다. 예매 방법·좌석·할인까지 구단이 한 장에 정리해 둔 안내가 그 자리에 맞다.
 */
export const TICKET_INFO_URL = 'https://www.hanwhaeagles.co.kr/ticketInfo.do';
