...
export const SEAT_GRADES: SeatGrade[] = [
  { id: 's1', name: '중앙 테이블석', price: 45000, remainRatio: 0.04, note: '2인 테이블 · 백네트 뒤' },
  { id: 's2', name: '1루 내야지정석', price: 18000, remainRatio: 0.22, note: '홈 응원석 · 육성 응원 구역' },
  { id: 's3', name: '3루 내야지정석', price: 18000, remainRatio: 0.61, note: '원정 응원석' },
  { id: 's4', name: '외야지정석', price: 12000, remainRatio: 0.48, note: '가족 단위 · 응원 소음 적음' },
...
export const GATES: GateInfo[] = [
  { name: '1번 게이트', serves: '1루 내야 · 중앙', crowd: 'high', note: '주차장에서 가장 가까워 줄이 깁니다' },
  { name: '3번 게이트', serves: '외야 · 잔디석', crowd: 'low', note: '돌아가지만 대기가 거의 없습니다' },
...
export function parkingAdvice(minutesToGameStart: number): ParkingAdvice[] {
  return PARKING_LOTS.map((lot) => {
    ...
    if (margin > 20) {
      status = 'open';
      summary = `만차까지 약 ${margin}분`;
    } else if (margin > -15) {
      status = 'tight';
      summary = `경기 ${lot.fullBeforeMinutes}분 전이면 만차`;
    } else {
      status = 'late';
      summary = '이미 만차일 가능성 높음';
    }
    return { lot, status, summary };
  }).sort((a, b) => {
    // 아직 갈 수 있는 곳을 위로, 그다음 가까운 순
    const rank = { open: 0, tight: 1, late: 2 };
    if (rank[a.status] !== rank[b.status]) return rank[a.status] - rank[b.status];
    return a.lot.walkMinutes - b.lot.walkMinutes;
  });
}
